...
MyeditableDTUI <- function(id){
  
  ns=NS(id)
  fluidPage(
    fluidRow(
      actionButton(ns("startsession"), "Refresh data"),
...
#' @param dataholder An object that has a srcnodes property containing the source table
...
#' @export
MyeditableDT <- function(input, output, session, dataholder=reactive(NULL),inputwidth=reactive(100), resetsession=reactive(FALSE)) {
  
  deleted<-deleted1<-edited<-edited1<-added<-added1<-updated1<-updated<-c()
  
  observe({
    updateTextInput(session,"result","")
  })
  ...
  editsessionstate = reactiveValues(
    reinitialize = FALSE,
    thedf = NULL,
    thedt = NULL
  ) 
  ...
  df=reactive({
    ...
    # browser()
    #cat("Result = ", input$result, " reinitialize = ", editsessionstate$reinitialize, "\n")
    if(input$result!="") {
      if (input$result == "updated" & editsessionstate$reinitialize == TRUE) {
        editsessionstate$reinitialize = FALSE
        deleted<-deleted1<-edited<-edited1<-added<-added1<-updated1<-updated<-c()
        updateTextInput(session,"result",value="")
        df = dataholder()$srcnodes
      } else {
    ...
    editsessionstate$thedf = df
    df
  })

  observeEvent(input$startsession, {
    #cat("Start session pressed - Result = ", input$result, "\n")
    editsessionstate$reinitialize = TRUE
  })
